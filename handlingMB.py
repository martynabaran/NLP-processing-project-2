...
class LanguageApp:
    ...
    def add_complement_be(self):
        ...
        label = tk.Label(self.master, text="Select an adjective or noun for the complement:")
        label.pack(pady=10)

        self.complement_selection = ttk.Combobox(self.master, values=adjectives + nouns)  # Połącz przymiotniki i rzeczowniki
        self.complement_selection.pack(pady=10)

        # Przycisk do potwierdzenia wyboru komplementu
        select_button = tk.Button(self.master, text="Select", command=self.confirm_complement)
    ...
    def confirm_complement(self):
        ...
        if 'complement' not in self.user_selection or not isinstance(self.user_selection['complement'], dict):
            self.user_selection['complement'] = {}
        ...
        if selected_complement.lower() not in adjectives:
            ...
            self.user_selection['complement']['subject'] = selected_complement
            print("Selected complement:",
                self.user_selection['subject'] if not self.is_complement else self.user_selection['complement'])            
            self.final_sentence()
            # self.final_sentence()  # Jeżeli to przymiotnik, od razu kończymy zdanie
    ...
    def add_noun_options(self):
        ...
        # Opcja dodania zaimka possessive
        self.possesive_check = tk.Checkbutton(self.master, text="Add a possessive", command=self.toggle_possesive)
        self.possesive_check.var = tk.BooleanVar()
        self.possesive_check["variable"] = self.possesive_check.var
        self.possesive_check.pack(pady=10)

        # Przycisk do zakończenia wyboru
        select_button = tk.Button(self.master, text="Confirm Noun", command=self.confirm_noun(arg=True))
        select_button.pack(pady=10)   
